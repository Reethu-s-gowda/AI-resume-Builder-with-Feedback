...
st.set_page_config(
    page_title="Resume Builder with Feedback",
    page_icon="🚀",
    layout="wide"
)
...
class ResumeApp:
    def __init__(self):
        ...
        self.pages = {
            ...
            "🔍 RESUME ANALYZER WITH FEEDBACK": self.render_analyzer,
       ...
       # Load Google Fonts and inject strict color override for form inputs
        # Load Google Fonts and inject strict color override for form inputs
        st.markdown("""
            <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;700&family=Poppins:wght@400;500;600&display=swap" rel="stylesheet">
            <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
            <style>
                /* 1. Small Top Boxes (Email, Phone, Links): Keep dark background, make text WHITE */
                ...
                /* 2. Large Boxes (Summary, Overviews, Achievements): Keep white background, make text BLACK */
    ...
    def render_builder(self):
        ...
        if st.button("Generate Resume 📄", type="primary"):
            ...
            st.write(f"Email input value: {st.session_state.get('email', '')}")
                ...
                try:
                    ...
                    if resume_buffer:
                        try:
                            ...
                            # Setup the file name cleanly inside the try block
                            current_name_val = st.session_state.form_data['personal_info']['full_name']
                            file_name_val = f"{current_name_val}_resume.docx"
                            
                            # Render the download button inside the try block
                            st.download_button(
    ...
    def render_about(self):
        ...
        st.markdown("""
            <div class="hero-section">
                <h1 class="hero-title">About Resume Builder with Feedback</h1>
        ...
        st.markdown("""
            <div class="vision-section">
                ...
                <p class="vision-text">
                    "Resume Builder with Feedback represents my vision of democratizing career advancement through technology.
    ...
    def render_analyzer(self):
        """Render the resume analyzer with feedback page"""
        apply_modern_styles()

        # Page Header
        page_header(
            "Resume Analyzer with Feedback",
        ...
        with analyzer_tabs[0]:
            ...
            if not uploaded_file:
                ...
                st.markdown(
                    self.render_empty_state("fas fa-cloud-upload-alt", "Upload your resume to get started with standard analysis"),
                    unsafe_allow_html=True
                )
            ...
            if uploaded_file:
                ...
                analyze_standard = st.button("🔍 Analyze My Resume", type="primary", use_container_width=True, key="analyze_standard_button")
                ...
                if analyze_standard:
                    with st.spinner("Analyzing your document..."):
                        ...
                        with col1:
                            ...
                            color_val = '#4CAF50' if ats_val >= 80 else '#FFA500' if ats_val >= 60 else '#FF4444'
                            status_val = 'Excellent' if ats_val >= 80 else 'Good' if ats_val >= 60 else 'Needs Improvement'
                            ...
                            <div class="feature-card">
                                ...
                                <div style="text-align: center; margin-top: 10px;">
                                    <span style="font-size: 1.2em; color: {color_val}; font-weight: bold;">
                                        {status_val}
                                    </span>
                                </div>
                            </div>
        ...
        with analyzer_tabs[1]:
            ...
            with st.expander("📊 AI Analyzer Statistics", expanded=False):
                try:
                    ...
                    if ai_stats["total_analyses"] > 0:
                        ...
                        with col2:
                            score_color = "#38ef7d" if ai_stats["average_score"] >= 80 else "#FFEB3B" if ai_stats["average_score"] >= 60 else "#FF5252"
                            st.markdown(f"""
                        ...
                        if ai_stats["recent_analyses"]:
                            ...
                            for rec in ai_stats["recent_analyses"]:
                                score = rec["score"]
                                ...
                                try:
                                    date_obj = datetime.datetime.strptime(rec["date"], "%Y-%m-%d %H:%M:%S")
                                ...
                                <tr>
                                    ...
                                    <td><div class="role-badge">{rec["job_role"]}</div></td>
                        ...
                        st.info("No AI analysis data available yet. Upload and analyze resumes to see statistics here.")
                except Exception as e:
            ...
            if not uploaded_file:
                # Display empty state with a prominent upload button
                st.markdown(
                    self.render_empty_state("fas fa-robot", "Upload your resume to get AI-powered analysis and recommendations"),
                    unsafe_allow_html=True
                )
            else:
                # Add a prominent analyze button
                analyze_ai = st.button("🤖 Analyze with AI", type="primary", use_container_width=True, key="analyze_ai_button")
                        ...
                        try:
                            ...
                            with st.spinner("🧠 AI is analyzing your resume..."):
                                ...
                                if use_custom_job_desc and custom_job_description:
                                    analysis_result = analyzer.analyze_resume_with_gemini(resume_text, job_role=job_role, job_description=custom_job_description)
                                ...
                                if analysis_result and "error" not in analysis_result:
                                    ...
                                    save_ai_analysis_data(None, {"model_used": selected_model, "resume_score": resume_score, "job_role": job_role})
                                ...
                                if analysis_result and "error" not in analysis_result:
                                    ...
                                    <div style="background-color: #262730; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
                                        ...
                                        <div style="display: flex; flex-wrap: wrap; gap: 20px;">
                                            <div style="flex: 1; min-width: 200px;">
                                                <p style="color: #ffffff;"><strong>Job Role:</strong> {job_role}</p>
                                    ...
                                    section_styles = {
                                        "## Overall Assessment": """<div class="report-section"><h3 style="background: linear-gradient(90deg, #1e3a8a, #3b82f6); color: white; padding: 10px; border-radius: 5px;"><i class="fas fa-chart-line"></i> Overall Assessment</h3><div class="section-content">""",
                                        "## Professional Profile Analysis": """<div class="report-section"><h3 style="background: linear-gradient(90deg, #047857, #10b981); color: white; padding: 10px; border-radius: 5px;"><i class="fas fa-user-tie"></i> Professional Profile Analysis</h3><div class="section-content">""",
                                        "## Skills Analysis": """<div class="report-section"><h3 style="background: linear-gradient(90deg, #4f46e5, #818cf8); color: white; padding: 10px; border-radius: 5px;"><i class="fas fa-tools"></i> Skills Analysis</h3><div class="section-content">""",
                                        "## Experience Analysis": """<div class="report-section"><h3 style="background: linear-gradient(90deg, #9f1239, #e11d48); color: white; padding: 10px; border-radius: 5px;"><i class="fas fa-briefcase"></i> Experience Analysis</h3><div class="section-content">""",
                                        "## Education Analysis": """<div class="report-section"><h3 style="background: linear-gradient(90deg, #854d0e, #eab308); color: white; padding: 10px; border-radius: 5px;"><i class="fas fa-graduation-cap"></i> Education Analysis</h3><div class="section-content">""",
                                        "## Key Strengths": """<div class="report-section"><h3 style="background: linear-gradient(90deg, #166534, #22c55e); color: white; padding: 10px; border-radius: 5px;"><i class="fas fa-check-circle"></i> Key Strengths</h3><div class="section-content">""",
                                        "## Areas for Improvement": """<div class="report-section"><h3 style="background: linear-gradient(90deg, #9f1239, #fb7185); color: white; padding: 10px; border-radius: 5px;"><i class="fas fa-exclamation-circle"></i> Areas for Improvement</h3><div class="section-content">""",
                                        "## ATS Optimization Assessment": """<div class="report-section"><h3 style="background: linear-gradient(90deg, #0e7490, #06b6d4); color: white; padding: 10px; border-radius: 5px;"><i class="fas fa-robot"></i> ATS Optimization Assessment</h3><div class="section-content">""",
                                        "## Recommended Courses": """<div class="report-section"><h3 style="background: linear-gradient(90deg, #5b21b6, #8b5cf6); color: white; padding: 10px; border-radius: 5px;"><i class="fas fa-book"></i> Recommended Courses</h3><div class="section-content">""",
                                        "## Resume Score": """<div class="report-section"><h3 style="background: linear-gradient(90deg, #0369a1, #0ea5e9); color: white; padding: 10px; border-radius: 5px;"><i class="fas fa-star"></i> Resume Score</h3><div class="section-content">""",
                                        "## Role Alignment Analysis": """<div class="report-section"><h3 style="background: linear-gradient(90deg, #7c2d12, #ea580c); color: white; padding: 10px; border-radius: 5px;"><i class="fas fa-bullseye"></i> Role Alignment Analysis</h3><div class="section-content">""",
                                        "## Job Match Analysis": """<div class="report-section"><h3 style="background: linear-gradient(90deg, #4d7c0f, #84cc16); color: white; padding: 10px; border-radius: 5px;"><i class="fas fa-handshake"></i> Job Match Analysis</h3><div class="section-content">""",
                                    }
                 
                                    for section, style in section_styles.items():
                                        if section in formatted_analysis:
                                            formatted_analysis = formatted_analysis.replace(section, style)
                                            next_section = False
    ...
    def render_home(self):
        ...
        with col2:
            if st.button("Get Started", key="get_started_btn", help="Click to start analyzing your resume", type="primary", use_container_width=True):
                cleaned_name = "🔍 RESUME ANALYZER WITH FEEDBACK".lower().replace(" ", "_").replace("🔍", "").strip()
    ...
    def main(self):
        ...
        with st.sidebar:
            ...
            st.title("Resume Builder with Feedback")
            st.markdown("---")
            ...
            if st.session_state.get('is_admin', False):
                ...
                with st.expander("👤 Admin Login"):
                    ...
                    if st.button("Login", key="login_button"):
                        try:
                            if admin_email_input == "admin@email.com" and admin_password == "password123":
